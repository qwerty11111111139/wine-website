...
document.addEventListener('DOMContentLoaded', function() {
    ...
    // === ЗАВАНТАЖЕННЯ ЗАМОВЛЕНЬ ДЛЯ АДМІНА ===
    loadAdminOrders();
    
    console.log('Адмін панель ініціалізована');

// --- Завантаження замовлень та рендер для Admin_order.html ---
async function loadAdminOrders() {
    const ordersBody = document.getElementById('ordersBody');
    if (!ordersBody) return; // на інших сторінках таблиці немає

    try {
        const resp = await fetch('api.php?action=get_orders', { method: 'GET', credentials: 'same-origin' });
        const data = await resp.json();

        if (!data || !data.success) {
            ordersBody.innerHTML = '<tr><td colspan="6">Не вдалося завантажити замовлення</td></tr>';
            return;
        }

        const orders = data.orders || [];
        if (orders.length === 0) {
            ordersBody.innerHTML = '<tr><td colspan="6">Замовлень поки немає</td></tr>';
            return;
        }
        ...
        orders.forEach(order => {
            ...
            tr.innerHTML = `
                <td>${order.id}</td>
                <td>${order.customer_name || order.user_name || ''}</td>
        ...
        console.error('Не вдалось завантажити замовлення для адміністратора', err);
        ordersBody.innerHTML = '<tr><td colspan="6">Помилка сервера</td></tr>';
    }
}
});
